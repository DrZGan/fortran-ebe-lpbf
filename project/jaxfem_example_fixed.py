...
def top(point):
    return np.isclose(point[2], Lz, atol=1e-6)

def bottom(point):
    return np.isclose(point[2], 0., atol=1e-6)

def walls(point):
    left = np.isclose(point[0], 0., atol=1e-6)
    right = np.isclose(point[0], Lx, atol=1e-6)
    front = np.isclose(point[1], 0., atol=1e-6)
    back = np.isclose(point[1], Ly, atol=1e-6)
